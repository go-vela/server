...
func (c *client) getOrg(org, path string) (*api.Secret, error) {
	return c.get(fmt.Sprintf("secret/%s/%s/%s", constants.SecretOrg, org, path))
}
...
func (c *client) getRepo(org, repo, path string) (*api.Secret, error) {
	return c.get(fmt.Sprintf("secret/%s/%s/%s/%s", constants.SecretRepo, org, repo, path))
}
...
func (c *client) getShared(org, team, path string) (*api.Secret, error) {
	return c.get(fmt.Sprintf("secret/%s/%s/%s/%s", constants.SecretShared, org, team, path))
}
