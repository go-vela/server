...
// pingCache is a helper function to send a "ping"
// request with backoff to the database.
//
// This will ensure we have properly established a
// connection to the Redis cache instance before
// we try to set it up.
func pingCache(ctx context.Context, c *Client) error {
	// attempt 10 times
	var err error
	for i := range 10 {
		...
		if err != nil {
			c.Logger.Debugf("unable to ping Redis cache. Retrying in %v", time.Duration(i)*time.Second)
	...
	return fmt.Errorf("unable to establish connection to Redis cache: %w", err)
}
...
func NewTest(installTokenKey string) (*Client, error) {
	...
	return New(
		...
		WithCluster(false),
		WithInstallTokenKey(installTokenKey),
	)
}
