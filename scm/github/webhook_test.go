...
func TestGithub_ProcessWebhook_Delete_Branch(t *testing.T) {
	// setup router
	s := httptest.NewServer(http.NotFoundHandler())
	defer s.Close()

	// setup request
	body, err := os.Open("testdata/hooks/delete_branch.json")
	if err != nil {
		t.Errorf("unable to open file: %v", err)
	}
	...
	wantBuild.SetSource("https://github.com/Codertocat")
	wantBuild.SetTitle("delete received from https://github.com/Codertocat/Hello-World")
	wantBuild.SetMessage("refs/heads/test branch deleted")
	wantBuild.SetSender("Codertocat")
	wantBuild.SetAuthor("Codertocat")
	wantBuild.SetEmail("")
	wantBuild.SetBranch("test")
	wantBuild.SetRef("main")
...
func TestGithub_ProcessWebhook_Delete_Tag(t *testing.T) {
	// setup router
	s := httptest.NewServer(http.NotFoundHandler())
	defer s.Close()

	// setup request
	body, err := os.Open("testdata/hooks/delete_tag.json")
	if err != nil {
		t.Errorf("unable to open file: %v", err)
	}
	...
	wantBuild.SetSource("https://github.com/Codertocat")
	wantBuild.SetTitle("delete received from https://github.com/Codertocat/Hello-World")
	wantBuild.SetMessage("refs/heads/test tag deleted")
	wantBuild.SetSender("Codertocat")
	wantBuild.SetAuthor("Codertocat")
	wantBuild.SetEmail("")
	wantBuild.SetBranch("test")
