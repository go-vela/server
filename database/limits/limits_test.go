...
func TestLimits_New(t *testing.T) {
	// setup types
...
func testPostgres(t *testing.T) (*Engine, sqlmock.Sqlmock) {
	...
	if err != nil {
		t.Errorf("unable to create new postgres limit engine: %v", err)
	}
...
func testSqlite(t *testing.T) *Engine {
	...
	if err != nil {
		t.Errorf("unable to create new sqlite limit engine: %v", err)
	}

	return _engine
}

// testOrgBuildLimit is a test helper function to create an api
// OrgBuildLimit type with all fields set to a fake value.
func testOrgBuildLimit() *api.OrgBuildLimit {
