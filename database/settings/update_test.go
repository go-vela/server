...
func TestSettings_Engine_UpdateSettings(t *testing.T) {
	...
	_mock.ExpectExec(`UPDATE "settings" SET "compiler"=$1,"queue"=$2,"repo_allowlist"=$3,"schedule_allowlist"=$4,"queue_restart_limit"=$5,"created_at"=$6,"updated_at"=$7,"updated_by"=$8 WHERE "id" = $9`).
		WithArgs(`{"clone_image":{"String":"target/vela-git-slim:latest","Valid":true},"template_depth":{"Int64":10,"Valid":true},"starlark_exec_limit":{"Int64":100,"Valid":true}}`,
			`{"routes":["vela","large"]}`, `{"octocat/hello-world"}`, `{"*"}`, 30, 1, testutils.AnyArgument{}, `octocat`, 1).
