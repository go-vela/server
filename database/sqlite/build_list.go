...
func (c *client) GetOrgBuildListByEvent(org string, exclude []int64, event string, page, perPage int) ([]*library.Build, int64, error) {
	...
	err = c.Sqlite.
		Table(constants.TableBuild).
		Raw(dml.ListOrgBuildsByEvent, org, exclude, event, perPage, offset).
		Scan(b).Error
...
// GetRepoBuildListByEvent gets a list of all builds by org name and event type from the database.
//
// nolint: lll // ignore long line length due to variable names
func (c *client) GetRepoBuildListByEvent(r *library.Repo, event string, page, perPage int) ([]*library.Build, int64, error) {
	logrus.Tracef("listing builds for org %s by event %s from the database", r.GetFullName(), event)
	...
	err = c.Sqlite.
		Table(constants.TableBuild).
		Raw(dml.ListRepoBuildsByEvent, r, event, perPage, offset).
		Scan(b).Error
