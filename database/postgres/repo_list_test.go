...
func TestPostgres_Client_GetOrgRepoList_LastUpdate(t *testing.T) {
	// setup types
	_repoOne := testRepo()
	_repoOne.SetID(1)
	_repoOne.SetUserID(1)
	...
	//
	// https://gorm.io/docs/sql_builder.html#DryRun-Mode
	_query := _database.Postgres.Session(&gorm.Session{DryRun: true}).Raw(dml.ListReposByLastUpdate, "foo", "10", "1").Statement
	...
	// ensure the mock expects the query for test case
	_mock.ExpectQuery(_query.SQL.String()).WillReturnRows(_rows)
