...
func TestInitStep_Engine_ListInitSteps(t *testing.T) {
	// setup types
	...
	// ensure the mock expects the query
	_mock.ExpectQuery(`SELECT * FROM "initsteps"`).WillReturnRows(_rows)
	...
	tests := []struct {
	...
	}{
		{
			...
			name:     "postgres",
			database: _postgres,
			want:     []*library.InitStep{_initStepOne, _initStepTwo},
		},
		{
			failure:  false,
			name:     "sqlite3",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.ListInitSteps()

			if test.failure {
				if err == nil {
					t.Errorf("ListInitSteps for %s should have returned err", test.name)
				}
			...
			if err != nil {
				t.Errorf("ListInitSteps for %s returned err: %v", test.name, err)
			}

			if !reflect.DeepEqual(got, test.want) {
				t.Errorf("ListInitSteps for %s is %v, want %v", test.name, got, test.want)
			}
		})
	}
}
