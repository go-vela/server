...
func TestTypes_Worker_ToAPI(t *testing.T) {
	...
	want.SetRunningBuilds([]*api.Build{rB})
	...
	// run test
	got := testWorker().ToAPI(want.GetRunningBuilds())
...
func TestTypes_Worker_WorkerFromAPI(t *testing.T) {
	// setup types
	rB := testutils.APIBuild()
	rB.SetID(1)
	...
	w.SetRunningBuilds([]*api.Build{rB})
