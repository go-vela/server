...
func TestTestReport_Engine_ListByBuild(t *testing.T) {
	// setup types
	...
	_rows := sqlmock.NewRows([]string{"id", "repo_id", "build_id", "created_at"}).
		AddRow(1, 1, 1, 1)
	_mock.ExpectQuery(`SELECT * FROM "testreports" WHERE build_id = $1 ORDER BY created_at DESC LIMIT $2`).
		WithArgs(1, 10).
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.ListTestReportsByBuild(ctx, _build, 1, 10)

			if test.failure {
				if err == nil {
					t.Errorf("ListTestReportsByBuild for %s should have returned err", test.name)
				}
			...
			if err != nil {
				t.Errorf("ListTestReportsByBuild for %s returned err: %v", test.name, err)
			}

			if len(got) != len(test.want) {
				t.Errorf("ListTestReportsByBuild for %s returned %d reports, want %d", test.name, len(got), len(test.want))
				return
			}
			...
			if len(got) > 0 {
				...
				if got[0].GetID() != test.want[0].GetID() ||
					...
					got[0].GetCreatedAt() != test.want[0].GetCreatedAt() {
					t.Errorf("ListTestReportsByBuild for %s returned unexpected report values: got %v, want %v",
