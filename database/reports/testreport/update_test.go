...
func TestTestReport_Engine_Update(t *testing.T) {
	// setup types
	...
	_mock.ExpectExec(`UPDATE "testreports" SET "build_id"=$1,"created_at"=$2 WHERE "id" = $3`).
		WithArgs(1, 1, 1).
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.UpdateTestReport(ctx, _testReport)
			...
			if !reflect.DeepEqual(got.GetID(), _testReport.GetID()) ||
				...
				!reflect.DeepEqual(got.GetCreatedAt(), _testReport.GetCreatedAt()) {
