...
func TestTestReport_Engine_CountTestReportsByRepo(t *testing.T) {
	// setup types
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			got, err := test.database.CountTestReportsByRepo(ctx, _repo, filters)
			...
			if err != nil {
				t.Errorf("CountTestReportsByRepo() unexpected error: %v", err)
			}

			if got != test.want {
				t.Errorf("CountTestReportsByRepo() got = %v, want %v", got, test.want)
			}
		})
	}
}
