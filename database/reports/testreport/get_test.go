// SPDX-License-Identifier: Apache-2.0

package testreport

import (
	"context"
	"reflect"
	"testing"

	api "github.com/go-vela/server/api/types"
	"github.com/go-vela/server/database/testutils"
	"github.com/go-vela/server/database/types"
)

func TestTestReport_Engine_GetTestReport(t *testing.T) {
	_testReport := testutils.APITestReport()
	_testReport.SetID(1)
	_testReport.SetBuildID(1)
	_testReport.SetCreatedAt(1)

	_postgres, _mock := testPostgres(t)
	defer func() { _sql, _ := _postgres.client.DB(); _sql.Close() }()

	// create expected result in mock
	_rows := testutils.CreateMockRows([]any{*types.TestReportFromAPI(_testReport)})

	// ensure the mock expects the query
	_mock.ExpectQuery(`SELECT * FROM "testreports" WHERE id = $1 LIMIT $2`).WithArgs(1, 1).WillReturnRows(_rows)

	_sqlite := testSqlite(t)
	defer func() { _sql, _ := _sqlite.client.DB(); _sql.Close() }()

	_, err := _sqlite.CreateTestReport(context.TODO(), _testReport)
	if err != nil {
		t.Errorf("unable to create test report for sqlite: %v", err)
	}

	// setup tests
	tests := []struct {
		failure  bool
		name     string
		database *Engine
		want     *api.TestReport
	}{
		{
			failure:  false,
			name:     "postgres",
			database: _postgres,
			want:     _testReport,
		},
		{
			failure:  false,
			name:     "sqlite3",
			database: _sqlite,
			want:     _testReport,
		},
	}

	// run tests
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.GetTestReport(context.TODO(), 1)

			if test.failure {
				if err == nil {
					t.Errorf("Get for %s should have returned err", test.name)
				}

				return
			}

			if err != nil {
				t.Errorf("Get for %s returned err: %v", test.name, err)
			}

			if !reflect.DeepEqual(got, test.want) {
				t.Errorf("Get for %s returned %v, want %v", test.name, got, test.want)
			}
		})
	}
}
