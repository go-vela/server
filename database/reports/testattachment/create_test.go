...
func TestEngine_CreateTestAttachment(t *testing.T) {
	...
	_mock.ExpectQuery(`INSERT INTO "testattachments" ("test_report_id","file_name","object_path","file_size","file_type","presigned_url","created_at","id") VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING "id"`).
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.CreateTestAttachment(context.TODO(), _testAttachment)
			...
			if !reflect.DeepEqual(got, _testAttachment) {
				t.Errorf("Create for %s returned %v, want %v", test.name, got, _testAttachment)
			}
		})
	}
}
