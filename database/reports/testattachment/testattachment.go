...
type (
	// config represents the settings required to create the engine that implements the TestAttachmentInterface interface.
	config struct {
		// specifies the encryption key to use for the TestAttachment engine
		EncryptionKey string
		// specifies to skip creating tables and indexes for the TestAttachment engine
		SkipCreation bool
	}
...
func New(opts ...EngineOpt) (*Engine, error) {
	// create new TestAttachment engine
	e := new(Engine)
	...
	err := e.CreateTestAttachmentTable(e.ctx, e.client.Config.Dialector.Name())
	if err != nil {
		return nil, fmt.Errorf("unable to create %s table: %w", constants.TableTestAttachment, err)
	}

	// create the indexes for the testattachments table
	err = e.CreateTestAttachmentIndexes(e.ctx)
	if err != nil {
		return nil, fmt.Errorf("unable to create indexes for %s table: %w", constants.TableTestAttachment, err)
	}
