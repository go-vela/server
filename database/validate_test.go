...
func TestDatabase_Config_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "success with partition configuration",
			failure: false,
			config: &config{
				Driver:              "postgres",
				Address:             "postgres://foo:bar@localhost:5432/vela",
				...
				SkipCreation:        false,
				LogPartitioned:      true,
				LogPartitionPattern: "logs_%",
				LogPartitionSchema:  "public",
			},
		},
		{
			name:    "success with partition disabled",
			failure: false,
			config: &config{
				Driver:              "postgres",
				Address:             "postgres://foo:bar@localhost:5432/vela",
				...
				EncryptionKey:       "A1B2C3D4E5G6H7I8J9K0LMNOPQRSTUVW",
				SkipCreation:        false,
				LogPartitioned:      false,
				LogPartitionPattern: "",
				LogPartitionSchema:  "",
			},
		},
		{
			name:    "failure with partition enabled but missing pattern",
			failure: true,
			config: &config{
				Driver:              "postgres",
				Address:             "postgres://foo:bar@localhost:5432/vela",
		...
		{
			name:    "failure with partition enabled but missing schema",
			failure: true,
			config: &config{
				Driver:              "postgres",
				Address:             "postgres://foo:bar@localhost:5432/vela",
		...
		{
			name:    "failure with partition enabled but missing both pattern and schema",
			failure: true,
			config: &config{
				Driver:              "postgres",
				Address:             "postgres://foo:bar@localhost:5432/vela",
