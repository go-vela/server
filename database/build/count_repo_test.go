...
func TestBuild_Engine_CountBuildsForUser(t *testing.T) {
	// setup types
	_buildOne := testBuild()
	_buildOne.SetID(1)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.CountBuildsForUser(_user, filters)

			if test.failure {
				if err == nil {
					t.Errorf("CountBuildsForUser for %s should have returned err", test.name)
				}
			...
			if err != nil {
				t.Errorf("CountBuildsForUser for %s returned err: %v", test.name, err)
			}

			if !reflect.DeepEqual(got, test.want) {
				t.Errorf("CountBuildsForUser for %s is %v, want %v", test.name, got, test.want)
			}
		})
	}
}
