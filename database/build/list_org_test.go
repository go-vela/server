...
func TestBuild_Engine_ListBuildsForOrg(t *testing.T) {
	...
	_buildTwo.SetHash("bar")
	_buildTwo.SetOrg("foo")
	_buildTwo.SetName("baz")
	_buildTwo.SetFullName("foo/baz")
	...
	_mock.ExpectQuery(`SELECT * FROM "builds" WHERE org = $1 ORDER BY name LIMIT 10`).WithArgs("foo").WillReturnRows(_rows)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, _, err := test.database.ListBuildsForOrg("foo", test.sort, filters, 1, 10)
