...
func TestBuild_Engine_GetBuildForOrg(t *testing.T) {
	// setup types
	_build := testBuild()
	_build.SetID(1)
	_build.SetUserID(1)
	...
	_mock.ExpectQuery(`SELECT * FROM "builds" WHERE org = $1 AND name = $2 LIMIT 1`).WithArgs("foo", "bar").WillReturnRows(_rows)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.GetBuildForOrg("foo", "bar")

			if test.failure {
				if err == nil {
					t.Errorf("GetBuildForOrg for %s should have returned err", test.name)
				}
			...
			if err != nil {
				t.Errorf("GetBuildForOrg for %s returned err: %v", test.name, err)
			}

			if !reflect.DeepEqual(got, test.want) {
				t.Errorf("GetBuildForOrg for %s is %v, want %v", test.name, got, test.want)
			}
		})
	}
}
