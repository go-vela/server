...
func TestDatabase_Engine_NewResources(t *testing.T) {
	...
	// ensure the mock expects the deployment queries
	_mock.ExpectExec(deployment.CreatePostgresTable).WillReturnResult(sqlmock.NewResult(1, 1))
	_mock.ExpectExec(deployment.CreateRepoIDIndex).WillReturnResult(sqlmock.NewResult(1, 1))
	...
	_mock.ExpectExec(secret.CreatePostgresAllowlistTable).WillReturnResult(sqlmock.NewResult(1, 1))
	_mock.ExpectExec(secret.CreatePostgresTable).WillReturnResult(sqlmock.NewResult(1, 1))
	_mock.ExpectExec(secret.CreateSecretID).WillReturnResult(sqlmock.NewResult(1, 1))
