...
func TestInit_Engine_CountInitsForBuild(t *testing.T) {
	// setup types
	_initOne := testInit()
	_initOne.SetID(1)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := test.database.CountInitsForBuild(_build)

			if test.failure {
				if err == nil {
					t.Errorf("CountInitsForBuild for %s should have returned err", test.name)
				}
			...
			if err != nil {
				t.Errorf("CountInitsForBuild for %s returned err: %v", test.name, err)
			}

			if !reflect.DeepEqual(got, test.want) {
				t.Errorf("CountInitsForBuild for %s is %v, want %v", test.name, got, test.want)
			}
		})
	}
}
