...
func TestMiddleware_RequestVersion(t *testing.T) {
	...
	gotVersion := context.Request.Header.Get("X-Vela-Version")

	if resp.Code != http.StatusOK {
		t.Errorf("RequestVersion returned %v, want %v", resp.Code, http.StatusOK)
	}

	if !reflect.DeepEqual(gotVersion, wantVersion) {
		t.Errorf("RequestVersion X-Vela-Version is %v, want %v", gotVersion, wantVersion)
	}
}
...
func TestMiddleware_ResponseVersion(t *testing.T) {
	...
	gotVersion := context.Writer.Header().Get("X-Vela-Version")

	if resp.Code != http.StatusOK {
		t.Errorf("ResponseVersion returned %v, want %v", resp.Code, http.StatusOK)
	}

	if !reflect.DeepEqual(gotVersion, wantVersion) {
		t.Errorf("ResponseVersion X-Vela-Version is %v, want %v", gotVersion, wantVersion)
	}
}
