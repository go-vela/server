...
func (c *client) SubstituteStages(s types.StageSlice) (types.StageSlice, error) {
	...
	for _, stage := range s {
		...
		st, err := c.SubstituteStage(stage)
		if err != nil {
			return nil, err
		}
		stage = st
...
func (c *client) SubstituteStage(s *types.Stage) (*types.Stage, error) {
	body, err := yaml.Marshal(s)
	if err != nil {
		return s, fmt.Errorf("unable to marshal configuration: %v", err)
	}

	// create substitute function
	subFunc := func(name string) string {
		// check for the environment variable
		env, ok := s.Environment[name]
		if !ok {
	...
	if err != nil {
		return s, fmt.Errorf("unable to substitute environment variables: %v", err)
	}

	// unmarshal step configuration
	err = yaml.Unmarshal([]byte(subStep), s)
	if err != nil {
		return s, fmt.Errorf("unable to unmarshal configuration: %v", err)
	}
