...
func (t *TemplateSlice) ToYAML() *yaml.TemplateSlice {
	// template slice we want to return
	templateSlice := new(yaml.TemplateSlice)

	// iterate through each element in the template slice
	for _, template := range *t {
		// append the element to the yaml template slice
		*templateSlice = append(*templateSlice, template.ToYAML())
	}
