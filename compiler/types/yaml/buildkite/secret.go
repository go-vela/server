...
func (s *StepSecretSlice) ToYAML() *yaml.StepSecretSlice {
	// step secret slice we want to return
	secretSlice := new(yaml.StepSecretSlice)

	// iterate through each element in the step secret slice
	for _, secret := range *s {
		// append the element to the yaml step secret slice
		*secretSlice = append(*secretSlice, &yaml.StepSecret{
			Source: secret.Source,
			Target: secret.Target,
		})
	}
...
func (o *Origin) ToYAML() yaml.Origin {
	return yaml.Origin{
		...
		Secrets:     *o.Secrets.ToYAML(),
		Pull:        o.Pull,
		Ruleset:     *o.Ruleset.ToYAML(),
	}
}
...
func (s *SecretSlice) ToYAML() *yaml.SecretSlice {
	// secret slice we want to return
	secretSlice := new(yaml.SecretSlice)

	// iterate through each element in the secret slice
	for _, secret := range *s {
		// append the element to the yaml secret slice
		*secretSlice = append(*secretSlice, secret.ToYAML())
	}
