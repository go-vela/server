...
func PostWebhook(c *gin.Context) {
	...
	if runAutoCancel &&
		...
		rBs, err := database.FromContext(c).ListPendingAndRunningBuildsForRepo(repo)
		if err != nil {
			logrus.Errorf("unable to fetch pending and running builds for %s: %v", repo.GetFullName(), err)
		}

		// call auto cancel routine
		err = autoCancel(c, b, rBs, repo, p.Metadata.AutoCancel.Pending, p.Metadata.AutoCancel.Running)
		if err != nil {
