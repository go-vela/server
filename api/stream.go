...
// swagger:operation POST /api/v1/repos/{org}/{repo}/builds/{build}/initsteps/{initstep}/stream stream PostInitStepStream
//
// Stream the logs for a initstep
//
// ---
// produces:
// - application/json
...
func PostInitStepStream(c *gin.Context) {
	...
	logger.Infof("streaming logs for initstep %s", entry)

	// send API call to capture the initstep logs
	_log, err := database.FromContext(c).GetLogForInitStep(i)
	if err != nil {
	...
	streamLog(c, logger, _log, "initstep", entry, r.GetTimeout())
